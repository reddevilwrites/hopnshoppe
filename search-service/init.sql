...
CREATE TABLE IF NOT EXISTS query_cache (
    ...
    -- Phase 3 columns ---------------------------------------------------------
    -- query_embedding: embedding of the normalized_query (BAAI/bge-base-en-v1.5, 768-dim).
...
ALTER TABLE query_cache ADD COLUMN IF NOT EXISTS query_embedding        VECTOR(768);
...
-- ── Migration guard: upgrade vector columns from 384-dim to 768-dim ──────────
-- Runs when the database was initialised with the old all-MiniLM-L6-v2 schema.
-- Dropping and re-adding is necessary because pgvector does not support altering
-- the dimension of an existing vector column.
-- query_cache: the cache is safe to wipe; embeddings will be regenerated on demand.
-- search_index: embeddings are re-generated by running the re_embed.py script after
--               the new image is deployed (see search-service/scripts/re_embed.py).
DO $$
DECLARE
  qc_dim integer;
  si_dim integer;
BEGIN
  -- pgvector stores the vector dimension in pg_attribute.atttypmod.
  -- vector(384) → atttypmod = 384;  vector(768) → atttypmod = 768.

  SELECT atttypmod INTO qc_dim
  FROM pg_attribute
  WHERE attrelid = 'query_cache'::regclass
    AND attname   = 'query_embedding'
    AND attnum    > 0;

  IF qc_dim IS NOT NULL AND qc_dim <> 768 THEN
    ALTER TABLE query_cache DROP COLUMN query_embedding;
    ALTER TABLE query_cache ADD  COLUMN query_embedding vector(768);
    UPDATE query_cache SET semantic_cache_enabled = FALSE;
  END IF;

  SELECT atttypmod INTO si_dim
  FROM pg_attribute
  WHERE attrelid = 'search_index'::regclass
    AND attname   = 'embedding'
    AND attnum    > 0;

  IF si_dim IS NOT NULL AND si_dim <> 768 THEN
    ALTER TABLE search_index DROP COLUMN embedding;
    ALTER TABLE search_index ADD  COLUMN embedding vector(768);
